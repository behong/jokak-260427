...
EMOTION_PATTERNS = [
    (["힘들", "지쳐", "버티", "포기"], "지쳐도 버티는 당신에게 필요한 말"),
    (["실패", "틀렸", "실수", "잘못"], "실패한 줄 알았을 때 다시 일어서는 말"),
    (["혼자", "외로", "고독"], "혼자인 것 같은 밤에 읽는 글"),
    (["자신", "나를", "자존", "나는"], "나를 잃지 않기 위해 기억할 말"),
    (["용서", "이해", "받아들"], "마음이 무거울 때 내려놓는 말"),
    (["성장", "노력", "꾸준", "루틴"], "조금씩 나아가고 싶은 날의 문장"),
    (["관계", "사람", "타인", "친구"], "사람에게 상처받았을 때 필요한 말"),
    (["슬픔", "눈물", "아프"], "아픈 마음을 조용히 안아주는 글"),
    (["행복", "감사", "소중"], "오늘을 조금 더 소중하게 만드는 말"),
]
...
BASE_TAGS = ["명언", "힐링", "좋은글귀", "지혜로운조각들", "짧은영상", "인생문장"]
TITLE_MAX_LENGTH = 70
...
def generate_title_local(quote: str) -> str:
    ...
    first = _first_sentence(text, 24)
    if first:
        return _clean_title(f"오늘 마음에 남겨둘 한 문장 | {first}…")
    return _clean_title("오늘 마음에 남겨둘 한 문장")
